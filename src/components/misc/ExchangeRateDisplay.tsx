    ...
    <Card className="glass-card animate-fade-in overflow-hidden">
      <CardHeader className="px-4 sm:px-6 py-4 sm:py-5">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
          ...
          >
            <RefreshCw
              className={`h-4 w-4 mr-2 transition-transform duration-500 ${isUpdating ? "animate-spin" : ""}`}
              aria-hidden="true"
            />
            ...
            .map(([currency, rate], index) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="text-xs text-muted-foreground">per 1 {currency}</div>
                </div>
              );
            })}
        </div>
